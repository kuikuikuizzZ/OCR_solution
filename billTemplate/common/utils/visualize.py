...
def draw_caption(image, box, caption,fontScale=2,thickness=2):
    ...
    """
    b = np.array(box).astype(int)
    cv2.putText(image, caption, (b[0], b[1]), cv2.FONT_HERSHEY_SIMPLEX, fontScale,
                (0, 0, 0), thickness)
...
#     setFont = ImageFont.truetype('', 20)
    ...
    Font = ImageFont.truetype(os.path.join(dirname,'HYDaSongJ.ttf'),
                         size)
    draw.text((box[0], box[1]), text,font=Font,fill=fillColor)
